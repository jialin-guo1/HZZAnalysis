...
parser.add_argument("-s","--step",dest="step",default= False, help="setup to true to run re-fit")
...
def draw_hist(diff_histogram,reco_histogram,gen_histogram,case,mass,path):
    ...
    gen_std_dev = round(gen_histogram.GetStdDev(),2)
    reco_std_dev = round(reco_histogram.GetStdDev(),2)
    diff_std_dev = round(diff_histogram.GetStdDev(), 2)
    dfff_std_dev_error = round(diff_histogram.GetStdDevError(),2)

    #get the max value for therr histogram and compare them to set the max value for the canvas
    max_gen = gen_histogram.GetMaximum()
    max_reco = reco_histogram.GetMaximum()
    max_diff = diff_histogram.GetMaximum()
    max_value = max(max_gen,max_reco,max_diff)

    #for each histogram set the max value
    gen_histogram.SetMaximum(max_value*1.2)
    reco_histogram.SetMaximum(max_value*1.2)
    diff_histogram.SetMaximum(max_value*1.2)
    ...
    text.DrawLatex(0.1,0.5,"Difference Std Dev Error: {}".format(dfff_std_dev_error/diff_std_dev))

    canvas.Update()
    canvas.SaveAs("{}/higgs_mass_comparison_{}_{}.png".format(path,mass,case))

def iteration_parm(a1,a2,n1,n2,mass):
    #fited root file 
    f = ROOT.TFile("2l2q_resolution_{}_{}.root".format(case_str[case],args.year),"read")
    #retrieve Parameter from fit result
    a1.SetVal(f.Get("a1").GetListOfFunctions().First().Eval(mass))
    a1.SetConstant(True)
    a2.SetVal(f.Get("a2").GetListOfFunctions().First().Eval(mass))
    a2.SetConstant(True)
    n1.SetVal(f.Get("n1").GetListOfFunctions().First().Eval(mass))
    n2.SetVal(f.Get("n2").GetListOfFunctions().First().Eval(mass))
...
#inputfile = ROOT.TFile("Histos_MakeShape_sig_{}.root".format(args.year))
inputfile = ROOT.TFile("FilesDeltaM/Hist_resolution{}.root".format(args.year))
...
for case in cases:
    ...
    for mass in massList:
        ...
        n1 = ROOT.RooRealVar('n1','',0.1,30)
        n2 = ROOT.RooRealVar('n2','',0.1,30)

        if args.step:
            print("[INFO] set parameter from previous fit")
            #iteration_parm(a1,a2,n1,n2,mass)
            #fited root file 
            f = ROOT.TFile("2l2q_resolution_{}_{}.root".format(case_str[case],args.year))
            #retrieve Parameter from fit result
            a1.setVal(f.Get("a1").GetListOfFunctions().First().Eval(mass))
            a1.setConstant(True)
            a2.setVal(f.Get("a2").GetListOfFunctions().First().Eval(mass))
            ...
            if args.year=='2017':
                pass
                #if case =='resolved':
                #    if mass>=1200 and mass<2000:
                #        a1 = ROOT.RooRealVar('a1','',0.2,10)
                #        a2 = ROOT.RooRealVar('a2','',0.2,10)
                #        n1 = ROOT.RooRealVar('n1','',0.2,25)
                #        n2 = ROOT.RooRealVar('n2','',2,2)
                #    elif mass>=2300:
                #        a1 = ROOT.RooRealVar('a1','',0.2,10)
                #        a2 = ROOT.RooRealVar('a2','',0.2,5)
                #        n1 = ROOT.RooRealVar('n1','',0.2,25)
                #        n2 = ROOT.RooRealVar('n2','',2,2)                 
                #    else:
                #        a1 = ROOT.RooRealVar('a1','',0.2,5)
                #        a2 = ROOT.RooRealVar('a2','',0.2,5)
                #        n1 = ROOT.RooRealVar('n1','',0.2,30)
                #        n2 = ROOT.RooRealVar('n2','',2,5)
                ...
                if mass == 1500 and case=='merged_tag':
                    ...
                    n1 = ROOT.RooRealVar('n1','',0.2,25)
                    n2 = ROOT.RooRealVar('n2','',2,2)
                if mass == 900 and case=='merged_tag':
                    a1 = ROOT.RooRealVar('a1','',0.2,10)
                    a2 = ROOT.RooRealVar('a2','',0.2,10)
                    n1 = ROOT.RooRealVar('n1','',0.2,25)
                    n2 = ROOT.RooRealVar('n2','',2,2)            
                ...
                if case=='merged_tag':
                    ...
                    if mass>=700 and mass<1900:
                        a1 = ROOT.RooRealVar('a1','',0.1,8)
                        a2 = ROOT.RooRealVar('a2','',0.1,8)
                        n1 = ROOT.RooRealVar('n1','',0.1,28)
                        n2 = ROOT.RooRealVar('n2','',2,2)
                    else:
                        a1 = ROOT.RooRealVar('a1','',0.1,4)
                        a2 = ROOT.RooRealVar('a2','',0.1,4)
                        n1 = ROOT.RooRealVar('n1','',0.1,10)
                        n2 = ROOT.RooRealVar('n2','',2,2)  
