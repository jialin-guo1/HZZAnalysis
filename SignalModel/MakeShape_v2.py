...
def reshape(hist,mean,width):
    h = ROOT.TH1D('h','h',400,0,4000); h.Sumw2()
...
out_file = ROOT.TFile('./Histos_MakeShape_{}_{}.root'.format(sample,year),'recreate')
out_file.cd()
        ...
        for tag in tags:
            ...
            for i,mass in enumerate(massList):
                if(massList[i]>0):
                    ...
                    h_gaus = ROOT.TH1D('{}_{}'.format(cutcat,mass),'{}_{}'.format(cutcat,mass),400,0,4000)
